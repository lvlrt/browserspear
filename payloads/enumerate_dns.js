    var dns_list = "216.58.211.164";
    var timeout = parseInt("1000");
    ...
    function notify() {
        console.log('result=Internal DNS found: '+ hostnames[cont]);
        check_next();
    }
    ...
    function check_next() {
        ...
        else setTimeout(function(){ console.log('result=DNS Enumeration done') }, 1000);
    }
    ...
    console.log(hostnames.length + ' hostnames loaded');
    if(do_resolv(protocol + hostnames[0] + ":" + port)==-1) {
        console.log('result=Browser not supported');
    }
